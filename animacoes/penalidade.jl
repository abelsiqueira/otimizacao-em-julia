...
function penalidade()
  ...
  for ρ = 2 .^ range(-8, 6, length=120)
    ϕ(x) = f(x) + ρ * sum(max.(0, A * x - b).^2)
    ϕ(x,y) = ϕ([x;y])
    ...
    xlims!(max(xg[1], x[1] - 1 / ρ), min(xg[end], x[1] + 1 / ρ))
    ylims!(max(yg[1], x[2] - 1 / ρ), min(yg[end], x[2] + 1 / ρ))

    frame(anim)
  end
  filename = joinpath(@__DIR__, "gif", "penalidade.gif")
